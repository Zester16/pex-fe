...
export default function NewspaperComponent(){
    ...
    async function addNewNewspaper(newsName,newsUrl,newsImage){
        try{
            ...
            const response = newtwork.addNewspaper(token,newsName,newsUrl,newsImage)
        }catch(error){
    ...
    <div>
        ...
        {addNewsState?<AddNewsPaperModel closeModel={closeAddNewsModel} addNewNewspaper={addNewNewspaper}/>:<></>}
    </div>)
}